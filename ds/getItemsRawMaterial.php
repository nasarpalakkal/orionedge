...
$Taxchk=$_POST['Taxchk'];																		
	...
	if($search=="")
	{
	$query = "SELECT A.item_no,A.item_descr,A.item_descr_ar,A.AvgCost,A.StandardCost,A.BaseLastCost,B.barcode,B.factor_val,D.descr as UnitDescr,B.sno UnitID,B.Packing_Setup,B.w_price as WholesalePrice,B.retail_price as RetailPrice,A.TaxType,C.perc,C.division_val,A.Type,Q.qty as QtyOnHand,A.IncludeTax FROM inventory as A left join inventory_uom as B on A.item_no=B.item_no left join tb_units as D on B.unit=D.id  left join tax_type as C on A.TaxType=C.id left join inventory_qty as Q on A.item_no=Q.item_no and Q.warehouse_id='".$whid."'" ;
	}
	else
	{
	$query = "SELECT A.item_no,A.item_descr,A.item_descr_ar,A.AvgCost,A.StandardCost,A.BaseLastCost,B.barcode,B.factor_val,D.descr as UnitDescr,B.sno UnitID,B.Packing_Setup,B.w_price as WholesalePrice,B.retail_price as RetailPrice,A.TaxType,C.perc,C.division_val,A.Type,Q.qty as QtyOnHand,A.IncludeTax FROM inventory as A left join inventory_uom as B on A.item_no=B.item_no left join tb_units as D on B.unit=D.id  left join tax_type as C on A.TaxType=C.id left join inventory_qty as Q on A.item_no=Q.item_no and Q.warehouse_id='".$whid."' WHERE  (A.item_no like'%".$search."%' or A.item_descr like '%".$search."%' or A.item_descr_ar like '%".$search."%' or B.barcode LIKE '%".$search."%')" ;
	}
