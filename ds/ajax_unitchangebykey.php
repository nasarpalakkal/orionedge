...
$sno=$_POST['sno'];
		...
		$productByCode = mysqli_query($link,"SELECT A.item_no,A.retail_price,B.item_descr,B.TaxType,B.IncludeTax,C.perc,C.division_val,D.qty,A.factor_val,A.unit from inventory_uom as A LEFT join inventory as B on A.item_no=B.item_no left JOIN tax_type C on B.TaxType=C.id left join inventory_qty as D on A.item_no=D.item_no LEFT JOIN tb_units as E on A.unit=E.id where A.item_no='$itemid' and A.unit!='$currentunit' and D.warehouse_id='$Whid'");
		$objectByCode = mysqli_fetch_array($productByCode);
		$newunit=$objectByCode['unit'];
		...
		mysqli_query($link,"update tbl_temp_sales set unit='$newunit',unit_price='$price',tax_amt='$Taxamt',total_price='$Total' where sno='$sno' and code='$itemid'");
...
<tr class=\"success\"><th><span class=\"ArFont\">".gettext("Item")."</span></th><th><span class=\"ArFont\">".gettext("Unit")."[F10]</span></th><th><span class=\"ArFont\">".gettext("Type")."[F10]</span></th><th><span class=\"ArFont\">".gettext("Price")."</span></th><th><span class=\"ArFont\">".gettext("Qty")."</span></th><th><span class=\"ArFont\">".gettext("Total")."</span></th><th><i class=\"fa fa-trash-o\"></i></th></tr></thead><tbody></table>";				
		...
		{		
				...
				<td align=\"center\"><input type=\"text\" value='$stype' id='SType$i' size=1  style=\"text-align:center;\" onKeyDown=\"return checkSalesType(event.key,'$i','$code','$unit','$tempid',event.which)\" autocomplete=\"off\"></td>	
