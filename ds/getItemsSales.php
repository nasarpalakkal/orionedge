...
$Taxchk=$_POST['Taxchk'];																		
	...
	if($search=="")
	{
			if($RoleID!=4)
			{
				$query = "SELECT A.item_no,A.item_descr,A.item_descr_ar,A.AvgCost,A.StandardCost,A.BaseLastCost,B.barcode,B.factor_val,D.descr as UnitDescr,B.sno UnitID,B.Packing_Setup,B.w_price as WholesalePrice,B.retail_price as RetailPrice,A.TaxType,C.perc,C.division_val,A.Type,Q.qty as QtyOnHand,A.IncludeTax FROM inventory as A left join inventory_uom as B on A.item_no=B.item_no left join tb_units as D on B.unit=D.id  left join tax_type as C on A.TaxType=C.id left join inventory_qty as Q on A.item_no=Q.item_no and Q.warehouse_id='".$whid."' WHERE Type=4 and A.store_id='$DefStore'" ;
			}
			else
			{
				$query = "SELECT A.item_no,A.item_descr,A.item_descr_ar,A.AvgCost,A.StandardCost,A.BaseLastCost,B.barcode,B.factor_val,D.descr as UnitDescr,B.sno UnitID,B.Packing_Setup,B.w_price as WholesalePrice,B.retail_price as RetailPrice,A.TaxType,C.perc,C.division_val,A.Type,Q.qty as QtyOnHand,A.IncludeTax FROM inventory as A left join inventory_uom as B on A.item_no=B.item_no left join tb_units as D on B.unit=D.id  left join tax_type as C on A.TaxType=C.id left join inventory_qty as Q on A.item_no=Q.item_no and Q.warehouse_id='".$whid."' WHERE Type=4 " ;
			}
	...
	else
	{
		if($RoleID!=4)
		{
			$query = "SELECT A.item_no,A.item_descr,A.item_descr_ar,A.AvgCost,A.StandardCost,A.BaseLastCost,B.barcode,B.factor_val,D.descr as UnitDescr,B.sno UnitID,B.Packing_Setup,B.w_price as WholesalePrice,B.retail_price as RetailPrice,A.TaxType,C.perc,C.division_val,A.Type,Q.qty as QtyOnHand,A.IncludeTax FROM inventory as A left join inventory_uom as B on A.item_no=B.item_no left join tb_units as D on B.unit=D.id  left join tax_type as C on A.TaxType=C.id left join inventory_qty as Q on A.item_no=Q.item_no and Q.warehouse_id='".$whid."' WHERE  A.Type=4 and (A.item_no like'%".$search."%' or A.item_descr like '%".$search."%' or A.item_descr_ar like '%".$search."%' or B.barcode LIKE '%".$search."%') and A.store_id='$DefStore'" ;
		}
		else
		{
	...
	if($row['QtyOnHand']!="") { $qtyonhand=round($row['QtyOnHand']/$row['factor_val']); } else {$qtyonhand=0;}
							...
							if($IncludeTax==1)
							{
								if($Taxchk==1)
								{
									...
									if($salestype==1){ $vatAmount=number_format(0,2,'.','');  } else{ $vatAmount=number_format(0,2,'.',''); }
									if($salestype==1){ $AmountwithTax=number_format($row['RetailPrice']/$division_val,2,'.','');  } else{ $AmountwithTax=number_format($row['WholesalePrice']/$division_val,2,'.',''); }
